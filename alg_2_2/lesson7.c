...
/* функция использует алгоритм блочной сортировки всего массива. Но при помещении содержимого корзин обратно
   в исходный массив проверяется чётность помещаемого элемента и чётность элемента, на чьё место помещаем.
   Таким образом, в корзинах сортируются все элементы массива, но обратно помещаются только чётные и только на те места,
   где изначально были чётные элементы. */
{
    ...
    free(buckets);              // освобождаем память, выделенную под массив
}
